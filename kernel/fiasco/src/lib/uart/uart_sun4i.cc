...
namespace L4
{
  ...
  bool Uart_sun4i::startup(Io_register_block const *regs)
  {
    _regs = regs;
  ...
  void Uart_sun4i::shutdown()
  {
  }

  bool Uart_sun4i::enable_rx_irq(bool enable)
  {
    _regs->write<unsigned int>(IER_REG, enable ? 1 : 0);
    return true;
  }
  
  bool Uart_sun4i::change_mode(Transfer_mode, Baud_rate r)
  {
    if (r != 115200)
      return false;
  ...
  int Uart_sun4i::get_char(bool blocking) const
  {
    while (!char_avail())
      if (!blocking)
	return -1;
  ...
  int Uart_sun4i::char_avail() const
  {
    return _regs->read<unsigned int>(LSR_REG) & LSR_REG_RX_FIFO_E_AVAIL;
  }

  void Uart_sun4i::out_char(char c) const
  {
  ...
  int Uart_sun4i::write(char const *s, unsigned long count) const
  {
